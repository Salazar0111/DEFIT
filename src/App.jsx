...
export default function App() {
  ...
  useEffect(() => {
    const f = lsGet(`fd_${tk()}`);
    if(f) {
      const todayFd = f.filter(item => !item.date || item.date === tk());
      setFood(todayFd);
    }
  ...
  const confirmFood = () => {
    if(!pending) return;
    const e = { id:Date.now(), date:tk(), name:pending.name, cal:pending.cal, pro:pending.pro,
  ...
  const addManual = () => {
    ...
    if(!name || !cal) return;
    const e = { id:Date.now(), date:tk(), name, cal:+cal, pro:+(pro||0), carb:+(carb||0), fat:+(fat||0),
